...
class QueryMode(Settings):
    ...
    def query_user(self, screen):
        ...
        self.input = self.font.render(self.mode.capitalize(), True, (0, 128, 0))
        self.input_rect = self.input.get_rect(center=(self.width/2, self.height/2))
        self.action = self.small_font.render("Press <R> for Rapid Mode or <T> for Traditional Mode", True, (0,128,0))
